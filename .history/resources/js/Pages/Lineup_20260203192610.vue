...
// Carousel Logic - Height increased for a more vertical/prominent look
...
<template>
    <div class="min-h-screen bg-black text-white font-sans pb-32">
        ...
        <div class="px-5 pt-6">
            <div class="relative mb-6 overflow-hidden rounded-xl aspect-[14/13] sm:aspect-[21/9] bg-zinc-900">
            ...
            <div class="relative mb-8">
                <input v-model="searchQuery" type="text" placeholder="Search artist or genre..." 
                    class="w-full bg-[#0e0e0e] border border-white/10 px-12 py-4 rounded-xl outline-none focus:border-[#ffcd00]/50 transition-all text-sm font-medium" />
            ...
            <div class="p-4 rounded-xl border border-[#ffcd00]/40 bg-gradient-to-b from-[#ffcd00]/5 to-transparent shadow-[0_0_20px_rgba(255,205,0,0.03)]">
                
                <div class="grid grid-cols-2 gap-4">
                    <div v-if="featuredArtist && !searchQuery" class="col-span-2 mb-2">
                        <div class="relative rounded-xl overflow-hidden border border-[#ffcd00]/50 bg-[#0c0c0c] h-[190px]">
                            <div class="relative z-10 flex h-full">
                                <div class="flex-1 p-6 flex flex-col justify-center">
                                    ...
                                    <div class="flex gap-2">
                                        <Link :href="`/artist/${featuredArtist.id}`" class="bg-white/5 border border-white/10 text-[9px] font-black uppercase px-5 py-2.5 rounded-lg">Profile</Link>
                                        <button v-if="featuredArtist.status === 'live'" @click="openVoting(featuredArtist)" class="bg-[#ffcd00] text-black text-[9px] font-black uppercase px-5 py-2.5 rounded-lg">Vote Now</button>
                                    </div>
                                </div>
                                <div class="w-2/5 h-full relative">
                                    <img :src="featuredArtist.image" class="w-full h-full object-cover" />
                                    <div class="absolute inset-0 bg-gradient-to-r from-[#0c0c0c] to-transparent"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-for="artist in displayArtists" :key="artist.id" 
                        class="relative p-1 rounded-xl border transition-all"
                        :class="artist.status === 'live' ? 'border-[#ffcd00] shadow-[0_0_15px_rgba(255,205,0,0.2)]' : 'border-zinc-800 bg-[#080808]'">
                        
                        <div class="relative aspect-square rounded-lg overflow-hidden mb-3">
                            <img :src="artist.image" class="w-full h-full object-cover" />
                            <div v-if="artist.status === 'upcoming'" class="absolute bottom-2 left-2 bg-black/70 backdrop-blur-md px-2 py-1 rounded-md border border-white/10">
