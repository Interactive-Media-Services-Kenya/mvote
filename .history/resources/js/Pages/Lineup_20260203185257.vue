...
<template>
    <div class="min-h-screen bg-brand-black text-white font-sans pb-32">
        ...
        <div class="px-6 pt-8 animate-fade-up">
            ...
            <div class="relative mb-10 group overflow-hidden rounded-[2rem] border border-white/10 shadow-2xl aspect-[16/9] sm:aspect-[21/9] bg-brand-gray/20">
                <div 
                ...
                <div class="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 flex gap-2">
                    <button 
                        ...
                        :class="currentSlide === i ? 'w-8 bg-brand-yellow shadow-[0_0_10px_#ffcd00]' : 'w-2 bg-white/20'"
                    ></button>
                </div>
            </div>
            ...
            <header class="mb-8 px-1">
                ...
                <div class="h-1 w-12 bg-brand-yellow mt-3 rounded-full"></div>
            </header>

            <div class="relative mb-10">
                <input v-model="searchQuery" type="text" placeholder="Search artist or genre..." class="w-full bg-brand-gray/30 border border-white/10 px-12 py-5 rounded-2xl outline-none focus:border-brand-yellow/50 transition-all font-medium text-white placeholder:text-gray-600 shadow-inner" />
            ...
            <div class="flex items-center gap-8 mb-8 border-b border-white/10 px-2">
                <button @click="activeTab = 'upcoming'" class="pb-4 text-[10px] font-black uppercase tracking-[0.2em] transition-all relative" :class="activeTab === 'upcoming' ? 'text-brand-yellow' : 'text-gray-500 hover:text-white'">
                    Lined Up
                    <div v-if="activeTab === 'upcoming'" class="absolute bottom-0 left-0 right-0 h-1 bg-brand-yellow shadow-[0_0_15px_#ffcd00]"></div>
                </button>
                <button @click="activeTab = 'closed'" class="pb-4 text-[10px] font-black uppercase tracking-[0.2em] transition-all relative" :class="activeTab === 'closed' ? 'text-brand-yellow' : 'text-gray-500 hover:text-white'">
                    Performed
            ...
            <div class="grid grid-cols-2 gap-4 sm:gap-6">
                <div v-if="featuredArtist && !searchQuery" class="col-span-2 mb-4 animate-fade-up">
                    <div class="relative group rounded-[2rem] overflow-hidden border border-white/10 glass-card h-[200px] sm:h-[240px] transition-all duration-500" :class="{ 'ring-1 ring-brand-yellow/30 shadow-[0_0_30px_rgba(255,205,0,0.1)]': featuredArtist.status === 'live' }">
                        ...
                        <div v-if="featuredArtist.status === 'live'" class="absolute inset-0 z-0 flex items-center justify-center opacity-20 pointer-events-none">
                            <div class="flex gap-1.5 items-end">
                                <div v-for="i in 15" :key="i" class="w-1.5 bg-brand-yellow rounded-full" :style="{ height: Math.random() * 60 + 40 + '%', animation: `hype-pulse ${Math.random() * 0.6 + 0.3}s infinite alternate` }"></div>
                            </div>
                        </div>

                        <div class="relative z-10 flex h-full items-center">
                            <div class="flex-1 p-6 sm:p-10 flex flex-col justify-center">
                                <p class="text-brand-yellow text-[9px] font-black uppercase tracking-[0.3em] mb-2 flex items-center gap-2">
                                    <span v-if="featuredArtist.status === 'live'" class="w-1.5 h-1.5 rounded-full bg-brand-yellow animate-ping"></span>
                                    {{ featuredArtist.status === "live" ? "Performing Now" : "Coming Up Next" }}
                                </p>
                                <h3 class="text-3xl sm:text-5xl font-black italic tracking-tighter uppercase leading-none mb-6 truncate">{{ featuredArtist.name }}</h3>
                                <div class="flex gap-3">
                                    <Link :href="`/artist/${featuredArtist.id}`" class="bg-white/5 hover:bg-white/10 text-white font-black py-3 px-6 rounded-xl uppercase text-[9px] tracking-widest border border-white/10 transition-all">Profile</Link>
                                    <button v-if="featuredArtist.status === 'live' && !featuredArtist.hasVoted" @click="openVoting(featuredArtist)" class="bg-brand-yellow text-black font-black py-3 px-6 rounded-xl uppercase text-[9px] tracking-widest shadow-xl hover:bg-white transition-all active:scale-95">Vote {{ timeRemaining }}</button>
                                </div>
                            </div>
                ...
                <div v-for="artist in displayArtists" :key="artist.id" class="group relative p-2 sm:p-3 rounded-[2rem] border border-white/5 bg-brand-gray/10 transition-all duration-500 hover:border-white/20" :class="[ artist.status === 'live' ? 'ring-1 ring-brand-yellow/40 bg-brand-yellow/5' : '' ]">
                    <div class="relative mb-4">
                        <Link :href="`/artist/${artist.id}`" class="block aspect-square rounded-[1.5rem] overflow-hidden shadow-2xl transition-transform group-hover:scale-[0.98]">
                            <img :src="artist.image" class="w-full h-full object-cover grayscale-[30%] group-hover:grayscale-0 transition-all duration-700" />
                            <div class="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/90 via-transparent to-transparent">
                                <div v-if="artist.hasVoted" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-green-500/20 backdrop-blur-md border border-green-500/30 text-[8px] uppercase font-black text-green-400">
        ...
        <div class="fixed bottom-10 left-1/2 -translate-x-1/2 w-[85%] max-w-sm z-40">
            <button v-if="liveArtist" @click="liveArtist.voting_started_at && !liveArtist.hasVoted ? openVoting(liveArtist) : null" :class="['w-full py-5 rounded-full font-black uppercase text-xs tracking-widest flex items-center justify-center gap-3 transition-all text-white shadow-[0_20px_50px_rgba(0,0,0,0.4)] transform active:scale-95', liveArtist.hasVoted ? 'bg-green-600' : 'bg-brand-yellow text-black']">
                <span v-if="!liveArtist.hasVoted" class="w-2 h-2 rounded-full bg-white animate-pulse"></span>
                <span class="truncate">{{ liveArtist.hasVoted ? 'Vote Recorded' : 'Vote Now: ' + liveArtist.name + timeRemaining }}</span>
            </button>
        </div>
