...
<template>
    <div class="min-h-screen bg-brand-black text-white font-sans pb-32">
        ...
        <div class="mx-3 mt-4 p-4 rounded-[2.5rem] border border-white/10 bg-brand-gray/5 shadow-2xl animate-fade-up">
            
            <div class="relative mb-8 group overflow-hidden rounded-2xl border border-white/10 shadow-2xl aspect-[14/10] sm:aspect-[21/9] bg-brand-gray/20">
                <div 
            ...
            <div class="relative mb-10">
                <input v-model="searchQuery" type="text" placeholder="Search artist or genre..." class="w-full bg-brand-black border border-white/10 px-12 py-5 rounded-2xl outline-none focus:border-brand-yellow/50 transition-all font-medium" />
            ...
            <div class="grid grid-cols-2 gap-4 sm:gap-6">
                <div v-if="featuredArtist && !searchQuery" class="col-span-2 mb-4">
                    <div class="relative group rounded-3xl overflow-hidden border-2 border-brand-yellow glass-card h-[190px] sm:h-[230px] shadow-[0_0_15px_rgba(255,205,0,0.2)]">
                        <div class="relative z-10 flex h-full items-center">
                            <div class="flex-1 p-6 flex flex-col justify-center">
                                ...
                                <div class="flex gap-2">
                                    <Link :href="`/artist/${featuredArtist.id}`" class="bg-white/10 hover:bg-white/20 text-white font-black py-2.5 px-5 rounded-xl uppercase text-[9px] tracking-widest border border-white/10">Profile</Link>
                                    <button v-if="featuredArtist.status === 'live' && !featuredArtist.hasVoted" @click="openVoting(featuredArtist)" class="bg-brand-yellow text-black font-black py-2.5 px-5 rounded-xl uppercase text-[9px] tracking-widest shadow-lg">Vote Now</button>
                                </div>
                            </div>
                            <div class="w-1/3 sm:w-2/5 h-full relative overflow-hidden">
                                <img :src="featuredArtist.image" class="w-full h-full object-cover" />
                                <div class="absolute inset-0 bg-gradient-to-r from-brand-black via-transparent to-transparent opacity-90"></div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div v-for="artist in displayArtists" :key="artist.id" 
                    class="group relative p-1 rounded-[1.8rem] border-2 transition-all duration-300"
                    :class="artist.status === 'live' ? 'border-brand-yellow shadow-[0_0_10px_rgba(255,205,0,0.3)]' : 'border-white/10 bg-brand-gray/5 hover:border-white/30'"
                >
                    <div class="relative mb-3">
                        <Link :href="`/artist/${artist.id}`" class="block aspect-square rounded-[1.5rem] overflow-hidden">
                            <img :src="artist.image" class="w-full h-full object-cover" />
                            <div v-if="artist.status === 'upcoming'" class="absolute top-2 left-2 px-3 py-1 bg-black/60 backdrop-blur-md rounded-lg text-[9px] font-black uppercase tracking-widest border border-white/10">Upcoming</div>
                        </Link>
                    </div>
                    <div class="px-2 pb-2">
                        <h3 class="font-bold text-base leading-tight truncate uppercase italic tracking-tighter">{{ artist.name }}</h3>
        ...
        <div class="fixed bottom-8 left-1/2 -translate-x-1/2 w-[85%] max-w-sm z-40">
            <button v-if="liveArtist" @click="liveArtist.voting_started_at && !liveArtist.hasVoted ? openVoting(liveArtist) : null" :class="['w-full py-5 rounded-full font-black uppercase text-sm tracking-widest flex items-center justify-center gap-2 transition-all shadow-2xl', liveArtist.hasVoted ? 'bg-green-600 text-white' : 'bg-brand-yellow text-black shadow-[0_15px_30px_rgba(255,205,0,0.4)]']">
