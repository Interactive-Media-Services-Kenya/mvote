...
<template>
    <div class="min-h-screen bg-brand-black text-white font-sans pb-32">
        ...
        <div class="mx-4 mt-6 p-5 rounded-[2.5rem] border border-white/10 bg-brand-gray/5 shadow-2xl animate-fade-up">
            
            <div class="relative mb-10 group overflow-hidden rounded-2xl border border-white/10 shadow-2xl aspect-[14/9] sm:aspect-[18/9] bg-brand-gray/20">
                <div 
            ...
            <header class="mb-8 px-2">
                ...
                <p class="text-gray-400 font-medium uppercase text-[10px] tracking-widest mt-1">Tonight's performing artists</p>
            </header>

            <div class="relative mb-10">
                <input v-model="searchQuery" type="text" placeholder="Search artist or genre..." class="w-full bg-brand-black/50 border border-white/10 px-12 py-5 rounded-2xl outline-none focus:border-brand-yellow/50 transition-all font-medium placeholder:text-gray-600" />
            ...
            <div class="grid grid-cols-2 gap-4 sm:gap-6">
                <div v-if="featuredArtist && !searchQuery" class="col-span-2 mb-4">
                    <div class="relative group rounded-[2rem] overflow-hidden border border-white/10 glass-card h-[200px] sm:h-[240px]" :class="{ 'ring-1 ring-brand-yellow/30': featuredArtist.status === 'live' }">
                        ...
                        <div class="relative z-10 flex h-full items-center">
                            <div class="flex-1 p-6 sm:p-10 flex flex-col justify-center">
                                ...
                                <h3 class="text-2xl sm:text-4xl font-black italic tracking-tighter uppercase leading-none mb-6 truncate">{{ featuredArtist.name }}</h3>
                                <div class="flex gap-2">
                                    <Link :href="`/artist/${featuredArtist.id}`" class="bg-white/10 hover:bg-white/20 text-white font-black py-2.5 px-5 rounded-xl uppercase text-[9px] tracking-widest border border-white/10 transition-all">Profile</Link>
                                    <button v-if="featuredArtist.status === 'live' && !featuredArtist.hasVoted" @click="openVoting(featuredArtist)" class="bg-brand-yellow text-black font-black py-2.5 px-5 rounded-xl uppercase text-[9px] tracking-widest shadow-lg active:scale-95 transition-all">Vote Now {{ timeRemaining }}</button>
                                </div>
                            </div>
                ...
                <div v-for="artist in displayArtists" :key="artist.id" class="group relative p-3 rounded-[2rem] border border-white/5 bg-brand-gray/10 transition-all duration-500" :class="[ artist.status === 'live' ? 'ring-1 ring-brand-yellow/40 scale-[1.02]' : 'hover:scale-[1.02] hover:border-white/20' ]">
                    <div class="relative mb-3">
                        <Link :href="`/artist/${artist.id}`" class="block aspect-square rounded-[1.5rem] overflow-hidden shadow-2xl transition-transform group-hover:scale-[0.98]">
                            ...
                            <div class="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/80 via-transparent to-transparent">
                                <div v-if="artist.hasVoted" class="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg bg-green-500/20 backdrop-blur-md border border-green-500/30 text-[9px] uppercase font-black text-green-400">
        ...
        <div class="fixed bottom-10 left-1/2 -translate-x-1/2 w-[85%] max-w-sm z-40">
            <button v-if="liveArtist" @click="liveArtist.voting_started_at && !liveArtist.hasVoted ? openVoting(liveArtist) : null" :class="['w-full py-5 rounded-full font-black uppercase text-sm tracking-widest flex items-center justify-center gap-2 transition-all text-white shadow-2xl', liveArtist.hasVoted ? 'bg-green-600' : 'bg-brand-yellow text-black shadow-[0_15px_30px_rgba(255,205,0,0.3)]']">
